  ...
  export class Dispatcher {
    public contextSwitch(): void {
      ...
      if (_Kernel.runningPcb) {
        ...
        _Kernel.krnTrace(
          `Dispatcher: Context switch OUT - Process ${
            _Kernel.runningPcb.pid
          } (State: ${_Kernel.runningPcb.state}, PC: 0x${_Kernel.runningPcb.pc
            .toString(16)
            .toUpperCase()
            .padStart(2, "0")})`
        );
        ...
        if (_Kernel.runningPcb.state !== pcbState.terminated) {
          ...
          _Kernel.krnTrace(
            `Dispatcher: Process ${_Kernel.runningPcb.pid} terminated and deallocated`
          );
        }
      }
      ...
      if (nextPcb) {
        ...
        _Kernel.krnTrace(
          `Dispatcher: Context switch IN - Process ${nextPcb.pid} (Base: ${
            nextPcb.base
          }, Limit: ${nextPcb.limit}, PC: 0x${nextPcb.pc
            .toString(16)
            .toUpperCase()
            .padStart(2, "0")})`
        );
    ...
    public checkAndSwitchIfNeeded(): void {
      if (
        ...
        _Kernel.krnTrace(
          `Dispatcher: Quantum (${_Scheduler.quantum} cycles) expired for Process ${_Kernel.runningPcb.pid}`
        );
