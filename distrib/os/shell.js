...
(function (TSOS) {
    class Shell {
        ...
        init() {
            ...
            this.commandList[this.commandList.length] = sc;
            sc = new TSOS.ShellCommand(this.shellChkdsk, "chkdsk", "[-recover|-reclaim|-defrag|-all] - Check disk and repair issues.");
            this.commandList[this.commandList.length] = sc;
            ...
            if (args.length > 0) {
                ...
                switch (topic) {
                    ...
                    case "link":
                        ...
                        break;
                    case "chkdsk":
                        _StdOut.putText("chkdsk - Check disk for errors and optionally repair.");
                        _StdOut.advanceLine();
                    ...
                    case "alias":
                        ...
                        _StdOut.advanceLine();
                        _StdOut.putText("With no arguments, displays all current aliases.");
                        _StdOut.advanceLine();
        ...
        shellKill(args) {
            ...
            _KernelInterruptQueue.enqueue(new TSOS.Interrupt(PSKILL, [pidToKill]));
            _StdOut.putText(`Error: Process ${pidToKill} not found.`);
        }
        shellKillAll(args) {
            if (_Kernel.runningPcb) {
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(PSKILL, [_Kernel.runningPcb.pid]));
            }
            for (const pcb of _Scheduler.residentList) {
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(PSKILL, [pcb.pid]));
            }
            for (const pcb of _Scheduler.readyQueue) {
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(PSKILL, [pcb.pid]));
            }
